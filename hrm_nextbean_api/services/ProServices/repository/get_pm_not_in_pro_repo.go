...
func (store *projectStore) GetPMNotInPro(pro_id string) ([]model.PM, error) {
	...
	rawsql := `select acc.id, acc.user_name, acc.email from account acc where acc.id not in (select account_id from project_manager where project_id=?) and acc.role = 'pm' and acc.deleted_at is null`
	data := []model.PM{}
