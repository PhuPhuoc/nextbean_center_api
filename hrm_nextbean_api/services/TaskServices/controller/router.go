...
func RegisterTaskRouter(r *mux.Router, db *sql.DB) {
	...
	task_router.HandleFunc("/{task-id}/start-task", middleware.TaskAccessMiddleware(db, false, false, true)(handleStartMyTask(db))).Methods("PUT")
	task_router.HandleFunc("/{task-id}/done-task", middleware.TaskAccessMiddleware(db, false, false, true)(handleDoneMyTask(db))).Methods("PUT")
}
