...
import (
	...
	"github.com/PhuPhuoc/hrm_nextbean_api/services/TechnicalServices/business"
	"github.com/PhuPhuoc/hrm_nextbean_api/services/TechnicalServices/model"
	"github.com/PhuPhuoc/hrm_nextbean_api/services/TechnicalServices/repository"
...
func handleCreateTech(db *sql.DB) func(rw http.ResponseWriter, req *http.Request) {
	return func(rw http.ResponseWriter, req *http.Request) {
		...
		store := repository.NewTechStore(db)
		biz := business.NewCreateTechBiz(store)
		if err_biz := biz.CreateTechBiz(info); err_biz != nil {
			utils.WriteJSON(rw, utils.ErrorResponse_DB(err_biz))
		}

		utils.WriteJSON(rw, utils.SuccessResponse_MessageCreated("Technical created successfully!"))
	}
}
